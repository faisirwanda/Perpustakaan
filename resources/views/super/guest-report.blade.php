    ...
    <style>
        ...
        <div>
            ...
            <div class="mt-5 d-flex justify-content-start">
                {{-- <a href="category-deleted" class="btn btn-lg btn-secondary me-3"><i class="bi bi-arrow-counterclockwise"></i> View Delete Data</a> --}}
                <a href="guest-export-super" class="btn btn-md btn-success"><i class="bi bi-filetype-xls"></i> Cetak</a>
            </div>
        </div>
    </div>
    <div class="d-flex justify-content-center">
        {{ $guest->appends(['work_id' => request('work_id'), 'month' => request('month')])->links() }}<br/>
    </div>
    @if ($guest->total() > $guest->perPage())
        <p class="text-center text-muted" style="font-size: 9pt">
            Menampilkan {{ $guest->firstItem() }} hingga {{ $guest->lastItem() }} dari {{ $guest->total() }} hasil
        </p>
    @endif
    <script>
